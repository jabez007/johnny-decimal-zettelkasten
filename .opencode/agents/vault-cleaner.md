...
- When consolidating, merge the `entities` and `tags` lists so no metadata is lost, and write the merged result with `obsidian-vault-mcp_obsidian_update_frontmatter` in batch mode.
