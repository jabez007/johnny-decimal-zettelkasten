...
4. **Synthesis Check:** Search the vault using `obsidian-vault-mcp_obsidian_rag_query` for existing overlaps.
5. **Proposal:** Suggest a new JDex entry or merging into an existing one. Include the structured YAML.
6. **Logging:** Use `obsidian-vault-mcp_obsidian_insert_at_heading` to log crystallization events back into the source daily note.
