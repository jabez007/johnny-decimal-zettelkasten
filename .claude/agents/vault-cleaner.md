...
- When consolidating, merge the `entities` and `tags` lists so no metadata is lost, and write the merged result with `mcp__obsidian-vault-mcp__obsidian_update_frontmatter` in batch mode.
