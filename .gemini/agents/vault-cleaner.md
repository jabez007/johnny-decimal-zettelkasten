...
- **Approval Gate:** You hold mutating tools. Propose first and wait for explicit approval before any `mcp_obsidian-vault-mcp_obsidian_replace_in_note`, `mcp_obsidian-vault-mcp_obsidian_move_note`, or `mcp_obsidian-vault-mcp_obsidian_update_frontmatter` call. If `mcp_obsidian-vault-mcp_obsidian_move_note` refuses because the destination exists, treat it as an ACID ID collision and re-propose; do not pass `overwrite`.
- **Link Integrity:** Consolidation must not break inbound links. Before retiring a note, find everything pointing at it with `mcp_obsidian-vault-mcp_obsidian_get_backlinks` and repoint each one at the surviving note with `mcp_obsidian-vault-mcp_obsidian_replace_in_note`. Do not leave a stub behind; there is no `redirect` status.
...
- When consolidating, merge the full frontmatter as described above — not only `entities` and `tags` — and write the result with `mcp_obsidian-vault-mcp_obsidian_update_frontmatter` in batch mode so untouched keys survive.
