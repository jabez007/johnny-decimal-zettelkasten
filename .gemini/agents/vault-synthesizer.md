...
- **The Knowledge Log:** Every synthesis session MUST be recorded in `_SYS/log.md`. Append with `mcp_obsidian-vault-mcp_obsidian_insert_at_heading`. If the log does not exist yet, propose creating it as part of the session's proposal, then append to it from then on.
