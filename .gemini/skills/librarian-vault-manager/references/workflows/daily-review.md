...
<purpose>
Extract durable knowledge from transient daily notes. Identify concepts that have earned a permanent place in the JDex through repeated mention, significant development, or explicit intent tags.
</purpose>

<workflow>
1. **Scan for intent tags**: Use `obsidian_search_notes` (or fallback search) to find instances of `#to-note` or `#to-process` across the `JRNL/` or inbox areas. These are explicitly flagged by the user for extraction.
2. **Scan recent daily notes**: Review `JRNL/YYYY/MM/YYYY-MM-DD.md` files from the past 7 days using `obsidian_read_note`.
3. **Extract distinct claims**: Identify specific observations, insights, or claims.
4. **Factor for atomicity**: Ensure each concept stands alone.
5. **Check existing entries**: For each extracted claim, use `obsidian_rag_query` on the permanent knowledge folders (e.g., `LIFE`) to find related entries or overlaps.
6. **Validate Indexability**: Ensure the proposed path and filename align with the filters in the relevant `.base` file (in `_SYS/`).
7. **Propose actions**:
   ...
   - **Refactoring Requirement**: When extracting text from a daily note, propose deleting the original text in the daily note and replacing it with an embed (`![[SYS.AC.ID]]`) pointing to the newly created permanent note. This leaves a breadcrumb while preventing duplication.
</workflow>
...
**Trigger**: [Found in daily note YYYY-MM-DD OR flagged with #to-note]
...
**Refactoring Action**:
I will extract the text from the daily note and replace it with:
`![[SYS.AC.ID-Descriptive Positive Title]]`
This preserves your journal's flow while securely storing the concept.
...
- Check tags: "I see you tagged this with #to-note. Let's give it a permanent home."
...
<constraints>
- Only review Markdown files in `JRNL/` directory or explicitly tagged notes.
...
Utilize block references/embeds to keep the original context connected to the new atomic note.
</goal>
