...
1. **Discovery:** Identify Systems (LIFE, WORK, PROJ), Areas, and Categories. Survey what already exists with `{{MCP_PREFIX}}obsidian_list_notes`, and confirm a proposed system prefix or ID is free with `{{MCP_PREFIX}}obsidian_search_notes` before claiming it.
...
3. **Initialization:** Once approved, create each file with `{{MCP_PREFIX}}obsidian_create_note`, which also creates any missing parent folders:
   ...
   Ensure every created note follows the graph-aware YAML standard. `{{MCP_PREFIX}}obsidian_create_note` refuses to overwrite an existing file. Treat a refusal as an ACID ID collision and pick the next free ID. Pass `overwrite: true` only after reading the destination and confirming with the user that it is genuinely stale — never as a way past a collision.
