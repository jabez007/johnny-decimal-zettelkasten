...
3. **Synthesis Search:** Search the vault using `{{MCP_PREFIX}}obsidian_rag_query` for existing themes and overlapping notes. Filter on the `entities` you extracted in step 2 to find exact prior coverage before falling back to an unfiltered semantic query.
