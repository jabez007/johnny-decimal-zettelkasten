...
- When consolidating, merge the `entities` and `tags` lists so no metadata is lost, and write the merged result with `{{MCP_PREFIX}}obsidian_update_frontmatter` in batch mode.
